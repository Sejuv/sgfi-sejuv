...
interface ImportCadastroDialogProps<T> {
  open: boolean
  ...
  onImportExcel: (file: File) => Promise<Omit<T, "id">[]>
}

export function ImportCadastroDialog<T>({
  open,
  onOpenChange,
  onImport,
  title,
  ...
  const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const items = await onImportExcel(file)
          ...
          <div className="border-t pt-4 space-y-3">
            ...
            <div className="flex flex-col gap-3">
              <input
                ...
                className="hidden"
                id="file-upload-cadastro"
              />
              
              <label htmlFor="file-upload-cadastro">
                <Button
                  type="button"
                  variant="outline"
